...
class ELO {
     ...
     */
    static private function eloProbability(float $elo1, float $elo2): float {
     ...
     */
    static public function eloRatingUpdate(float $elo1, float $elo2, bool $team1Won = true, int $K = 30) {
        $pB = self::eloProbability($elo1, $elo2);
        $pA = self::eloProbability($elo2, $elo1);
        if ($team1Won) {
            $elo1 += $K * (1 - $pA);
            $elo2 += $K * (0 - $pB);
        }
